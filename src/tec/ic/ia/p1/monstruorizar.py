...
def get_datos_arboles(muestras):
    salida = []
    muestrasT = numpy.transpose(muestras)
    #transformarlo a numeros
    label_encoder = LabelEncoder()
    ...
    #normalizar edad
    edad = pd.qcut(stringToFloat(muestrasT[2]), 6, labels=["Nino", "Adolecente", "Joven", "Adulto Joven", "Adulto", "Adulto Mayor"])[numpy.newaxis, :].T ##categotiza valor continuo
    ...
    escolaridad = pd.qcut(stringToFloat(muestrasT[8]), 5, labels=["Muy Baja","Baja", "Media", "Alta", "Muy Alta"])[numpy.newaxis, :].T#(muestrasT[8])[numpy.newaxis, :].T##transpuesta
    ...
    poblacion = pd.qcut(stringToFloat(muestrasT[15]), 3, labels=["Muy poco poblado", "Poco poblado", "Medianamente poblado", "Altamente Poblado"])[numpy.newaxis, :].T##transpuesta
    superficie = pd.qcut(stringToFloat(muestrasT[16]), 5, labels=["Muy pequena","Pequena","Mediana", "Grande", "Muy Grande"])[numpy.newaxis, :].T##transpuesta
    densidad = pd.qcut(stringToFloat(muestrasT[17]),4, labels=["Muy poco denso", "Poco denso", "Medianamente denso", "Muy denso"])[numpy.newaxis, :].T##transpuesta
    vOcupadas = pd.qcut(stringToFloat(muestrasT[18]), 4, labels=["Muy pocas","Pocas","Regular","Muchas"])[numpy.newaxis, :].T##transpuesta
    ocupantes = pd.qcut(stringToFloat(muestrasT[19]), 4, labels=["Bajo el promedio","promedio","Sobre el promedio"])[numpy.newaxis, :].T##transpuesta
