...
def prueba(n, capas, unidadCapas, activacion):
	...
	# Fit the model
	model.fit(X[:8000], encoded[:8000], epochs=150, batch_size=10)
	# evaluate the model
	scores = model.evaluate(X[2000:], encoded[2000:])
	print(scores)
	print("\n%s: %.2f%%" % (model.metrics_names[1], scores[1]*100))


def sustituir():
	muestras = generar_muestra_pais(100)
	...
	for filas in range(len(muestras)):
		salida.append([])
		for i in range(1, len(muestras[filas]),1):
	...
	createCSV("refactorizado.csv",salida)
	return salida
#sustituir()
prueba(500, 5,8,'relu')
